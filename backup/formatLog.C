...
void formatLog() {
  ...
  FILE *f3 = fopen(Form("%s", "allFormat.dat"), "w+");
  if (f2 == NULL) {
  ...
  for(i = 0; i <= strlen(hour); i++) if(hour[i] == ':') hour[i] = '_';
  ...
  FILE *f4 = fopen(Form("%s", "allFormat.dat"), "r");
  if (f2 == NULL) {
    printf("Cannot find file allFormat.dat\n");
    return;
  }

  FILE *f5 = fopen(Form("%s", "allPlot.dat"), "w+");
  if (f2 == NULL) {
    printf("Cannot find file allPlot.dat\n");
    return;
  }
  fscanf(f4, "%d %d %d %s %s %s %s %s", &counts, &muons, &neutrons, weekday, month, day, hour, year); //skip first two lines

  fprintf(f5, "%s_%s_%s__%s\n", year, month, day, hour);
  fprintf(f5, "%d\n", counts);
  
  fscanf(f4, "%d %d %d %s %d %d %d %d %d %d", &counts, &muons, &neutrons, weekday, &monthInt, &dayInt, &hourInt, &minuteInt, &secondInt, &yearInt); //skip first two lines
  ...
  while (1) {
    ...
    //  fprintf(f55, "%d %d %d %s %d %d %d %d %d %d\n", counts, muons, neutrons, weekday, monthInt, dayInt, hourInt, minuteInt, secondInt, yearInt);
